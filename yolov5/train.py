...
def _build_class_filter(names, exclude_class_names: str, single_cls: bool):
    """Build class filter metadata from class names to exclude."""
    if single_cls:
        return None

    excludes = [x.strip().lower() for x in str(exclude_class_names).split(",") if x.strip()]
    if not excludes:
        return None

    names_list = names if isinstance(names, list) else [names[k] for k in sorted(names.keys())]
    excluded_ids = {i for i, n in enumerate(names_list) if str(n).strip().lower() in excludes}
    if not excluded_ids:
        return None

    keep_ids = [i for i in range(len(names_list)) if i not in excluded_ids]
    if not keep_ids:
        raise ValueError("All classes were excluded. Please adjust --exclude-class-names.")

    id_map = {old_id: new_id for new_id, old_id in enumerate(keep_ids)}
    kept_names = [names_list[i] for i in keep_ids]
    return {"keep_ids": keep_ids, "id_map": id_map, "kept_names": kept_names, "excluded_ids": sorted(excluded_ids)}


def _apply_class_filter_to_dataset(dataset, class_filter):
    """Filter/remap dataset labels in memory according to class_filter."""
    keep_ids = set(class_filter["keep_ids"])
    id_map = class_filter["id_map"]
    dropped = 0
    ...
    for i, labels in enumerate(dataset.labels):
        ...
        if len(filtered):
            remapped_cls = np.array([id_map[int(c)] for c in filtered[:, 0]], dtype=filtered.dtype)
            filtered[:, 0] = remapped_cls
        dataset.labels[i] = filtered

        if hasattr(dataset, "segments") and i < len(dataset.segments):
            segs = dataset.segments[i]
            if isinstance(segs, list) and len(segs) == len(mask):
                dataset.segments[i] = [seg for seg, m in zip(segs, mask) if m]
...
def train(hyp, opt, device, callbacks):
    ...
    # CoordAttention uses adaptive_avg_pool2d in backward, which has no deterministic CUDA impl.
    init_seeds(opt.seed + 1 + RANK, deterministic=False)
    with torch_distributed_zero_first(LOCAL_RANK):
        data_dict = data_dict or check_dataset(data)  # check if None
    train_path, val_path = data_dict["train"], data_dict["val"]
    names = {0: "item"} if single_cls and len(data_dict["names"]) != 1 else data_dict["names"]  # class names
    class_filter = _build_class_filter(names, opt.exclude_class_names, single_cls)
    if class_filter:
        names = class_filter["kept_names"]
        data_dict["names"] = names
        data_dict["nc"] = len(names)
        LOGGER.info(
            f"Filtering classes online, excluded ids={class_filter['excluded_ids']}, keeping classes={names}"
        )
    ...
    if class_filter:
        dropped = _apply_class_filter_to_dataset(dataset, class_filter)
        LOGGER.info(f"Train labels filtered online, dropped {dropped} objects from excluded classes")

    labels = np.concatenate(dataset.labels, 0) if len(dataset.labels) else np.zeros((0, 5), dtype=np.float32)
    if labels.size:
    ...
    if RANK in {-1, 0}:
        ...
        if class_filter:
            val_dropped = _apply_class_filter_to_dataset(val_dataset, class_filter)
            LOGGER.info(f"Val labels filtered online, dropped {val_dropped} objects from excluded classes")
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Comma-separated class names to exclude online during training/validation",
    )
