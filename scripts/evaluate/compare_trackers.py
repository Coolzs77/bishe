...
class TrackerComparator:
    ...
    def print_comparison_results(self, all_results):
        ...
        try:
            # 按MOTA排序找出最优
            mota_ranking = sorted(all_results, key=lambda x: x.get('metrics', {}).get('MOTA', 0) or 0, reverse=True)
            if mota_ranking:
                print(f'  - 综合性能最优 (MOTA): {mota_ranking[0]["tracker"]} ({mota_ranking[0].get("metrics", {}).get("MOTA", "N/A")})')
            
            # 按IDF1排序找出身份保持最优
            idf1_ranking = sorted(all_results, key=lambda x: x.get('metrics', {}).get('IDF1', 0) or 0, reverse=True)
            if idf1_ranking:
                print(f'  - 身份保持最优 (IDF1): {idf1_ranking[0]["tracker"]} ({idf1_ranking[0].get("metrics", {}).get("IDF1", "N/A")})')
