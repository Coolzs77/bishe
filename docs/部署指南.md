...
```bash
# 安装交叉编译工具链（RV1126使用32位ARM Cortex-A7）
...
# 传输模型
scp ../../outputs/weights/best.rknn root@rv1126:/opt/infrared_tracker/model/
...
./infrared_tracker --model model/best.rknn --source /dev/video0
```
...
```bash
# 使用nohup后台运行
nohup ./infrared_tracker --model model/best.rknn > tracker.log 2>&1 &
