...
# ============ 红外图片检测 ============
# 默认批量跑 model/ 下所有 test_*.jpg/png/bmp，结果写到 ./outputs/
# 基线 
./bishe_rknn_detect model/best_baseline.rknn
# 基线+eiou 模型 (主力)
./bishe_rknn_detect model/best_eiou.rknn
# ghost+eiou 模型 (轻量化候选)
./bishe_rknn_detect model/best_ghost_eiou.rknn

# 单图模式
./bishe_rknn_detect model/best_eiou.rknn model/test_00.jpg model/infrared_labels.txt out.png

# 显式目录批量模式（跑该目录下全部图片）
./bishe_rknn_detect model/best_eiou.rknn model/ model/infrared_labels.txt outputs/

# ============ 红外视频检测 ============
./bishe_rknn_video model/best_baseline.rknn model/ZAtDSNuZZjkZFvMAo_seq006.mp4 \
...
./bishe_rknn_video model/best_ghost_eiou.rknn model/ZAtDSNuZZjkZFvMAo_seq006.mp4 \
    model/infrared_labels.txt out_video.mp4
...
# EIoU 模型推荐阈值（降低板端冗余框）
./bishe_rknn_video model/best_eiou.rknn model/ZAtDSNuZZjkZFvMAo_seq006.mp4 \
    model/infrared_labels.txt out_video.mp4 0.55 0.45

# 关闭默认的时序对齐跟踪 (track=0); 默认 track=1 与 PC 检测+跟踪链路更一致
./bishe_rknn_video model/best_eiou.rknn model/ZAtDSNuZZjkZFvMAo_seq006.mp4 \
    model/infrared_labels.txt out_video.mp4 0.25 0.45 0

# ──────────────────────────────────────────────────────────
# 纯计算 FPS 基准测试（输出路径填 - 跳过磁盘写入，overlay=0 跳过绘制）
# 目标：计算 FPS (推理+跟踪) >= 25fps
# ──────────────────────────────────────────────────────────
# 完整计算管道（推理+跟踪，无绘制/写入）
./bishe_rknn_video model/best_eiou.rknn model/ZAtDSNuZZjkZFvMAo_seq006.mp4 \
    model/infrared_labels.txt - 0.55 0.35 1 0

# 含绘制，无写入（验证绘制开销）
./bishe_rknn_video model/best_eiou.rknn model/ZAtDSNuZZjkZFvMAo_seq006.mp4 \
    model/infrared_labels.txt - 0.55 0.35 1 1

# 参数说明: <model> <视频> <标签> <输出|-=skip> [conf] [nms] [track:0/1] [overlay:0/1]
# 输出示例:
#   计算 FPS (推理+跟踪): 27.3  [✓ >=25fps]
#   端到端 FPS (含读写):  22.1

# 把输出的视频和图片拉回本地
adb pull /userdata/bishe_rknn_yolov5/out_video.mp4 /home/coolzs77/bishe/outputs/    
scp coolzs77@192.168.11.128:/home/coolzs77/bishe/outputs/* D:/pythonPro/bishe/outputs/
...
红外图片已加载: ./model/test_00.jpg (640x512, format=0)
检测到 3 个目标:
  [0] person @ (120,80)-(200,300) 87.3%
  [1] person @ (350,100)-(420,310) 72.1%
  [2] car @ (50,250)-(280,400) 65.8%

结果已保存: ./outputs/test_00_out.png
...
批量完成: success=5 fail=0
```
...
A: 优先使用更保守阈值 `conf=0.55, nms=0.45`。本项目板端程序已对 `best_eiou*.rknn` 提高默认 `conf` 到 `0.55`（命令行可覆盖），用于抑制 INT8 下阈值附近的冗余候选框。
